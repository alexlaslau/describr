    ...
    return (
        <>
            ...
            <div className="min-h-screen relative overflow-hidden bg-gray-950 text-gray-100">
                ...
                <div className="relative z-10 mx-auto flex min-h-screen w-full max-w-7xl flex-col px-4 pb-10 pt-4 sm:px-8 sm:pt-6 lg:px-12">
                    <main className="mt-8 flex-1 sm:mt-10">
                        <section className="grid items-start gap-8 lg:grid-cols-12 lg:gap-10">
                            <div className="lg:col-span-7">
                                <img src="/logo.png" alt="Describr logo" className="mx-auto mb-4 h-auto w-[min(41vw,20.7rem)]" />

                                <div className="inline-flex items-center gap-2 rounded-full border border-indigo-300/30 bg-indigo-300/10 px-4 py-1.5 text-xs font-medium uppercase tracking-[0.12em] text-indigo-200">
                                    AI-Accelerated Marketing Copy
                                </div>

                                <h1 className="mt-5 max-w-3xl text-3xl font-semibold leading-tight text-white sm:mt-6 sm:text-5xl lg:text-6xl">
                                ...
                                <div className="mt-7 flex flex-wrap items-center gap-3 sm:mt-8">
                                    {isLoggedIn ? (
                                        <Link
                                            ...
                                            className="rounded-xl bg-indigo-600 px-6 py-3 text-sm font-semibold text-white transition hover:translate-y-[-1px] hover:bg-indigo-700"
                                        >
                                            Go to Dashboard
                                        </Link>
                                    ) : (
                                        <>
                                            {canRegister && (
                                                <Link
                                                    href={route('register')}
                                                    className="rounded-xl bg-indigo-600 px-7 py-3.5 text-base font-semibold text-white transition hover:translate-y-[-1px] hover:bg-indigo-700"
                                                >
                                                    Get Started
                                                </Link>
                                            )}
                                            {canLogin && (
                                                <Link
                                                    href={route('login')}
                                                    className="rounded-xl border border-white/20 px-7 py-3.5 text-base font-medium text-gray-100 transition hover:border-white/40 hover:bg-white/10"
                                                >
                                                    Log in
                                                </Link>
                                            )}
                            ...
                            <div className="lg:col-span-5 lg:pl-2">
                                {isLoggedIn ? (
                                    <div className="rounded-3xl border border-white/15 bg-white/5 p-6 shadow-2xl shadow-indigo-950/30 backdrop-blur-sm transition duration-300 hover:border-indigo-300/40">
                                        <p className="text-xs uppercase tracking-[0.16em] text-gray-300">Welcome back</p>
                                        <h2 className="mt-2 text-2xl font-semibold text-white">{auth.user.name}</h2>
                                        ...
                                        <Link
                                            href={route('products.index')}
                                            className="mt-6 inline-flex rounded-xl bg-white px-4 py-2.5 text-sm font-semibold text-gray-900 transition hover:bg-gray-100"
                                        >
                                            Open Dashboard
                                        </Link>
                                    </div>
                        ...
                        <section className="mt-14 sm:mt-20">
                            <div className="mb-8 flex items-center gap-3">
                                ...
                                <p className="text-xs font-medium uppercase tracking-[0.2em] text-gray-400">How It Works</p>
                                <div className="h-px flex-1 bg-white/15" />
                            </div>
                            ...
                            <div className="grid gap-3 sm:gap-4 md:grid-cols-3">
                                {howItWorks.map((step, index) => (
                                    <article
                                        key={step.title}
                                        className="group rounded-2xl border border-white/10 bg-white/5 p-6 transition duration-300 hover:-translate-y-1 hover:border-indigo-300/40 hover:bg-white/10"
                                    >
                        ...
                        <section className="mt-12 sm:mt-16">
                            <div className="mb-6 flex items-center justify-between gap-6 sm:mb-8">
                                <div>
                                    <p className="text-xs font-semibold uppercase tracking-[0.18em] text-gray-400">Feature Highlights</p>
                            ...
                            <div className="grid gap-4 sm:grid-cols-2">
                                {features.map((feature) => (
                                    <article
                                        key={feature.title}
                                        className="rounded-2xl border border-white/10 bg-white/[0.04] p-6 transition duration-300 hover:border-indigo-300/40 hover:bg-white/10"
                                    >
